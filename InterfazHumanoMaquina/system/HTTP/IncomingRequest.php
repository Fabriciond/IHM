...
class IncomingRequest extends Request
{
    ...
    private function getPostMaxSize(): float
    {
        $postMaxSize = ini_get('post_max_size');

        switch (strtoupper(substr($postMaxSize, -1))) {
            case 'G':
                $postMaxSize = (float) str_replace('G', '', $postMaxSize) * 1024 ** 3;
                break;

            case 'M':
                $postMaxSize = (float) str_replace('M', '', $postMaxSize) * 1024 ** 2;
                break;

            case 'K':
                $postMaxSize = (float) str_replace('K', '', $postMaxSize) * 1024;
                break;

            default:
                $postMaxSize = (float) $postMaxSize;
        }
